...
async function createIndexes() {
  try {
    let sft_Collection = db.collection("sft-posts");

    // Create indexes in parallel for better performance
    await Promise.all([
      sft_Collection.createIndex({ posttitle: 1 }, { background: true }),
      sft_Collection.createIndex({ postdetails: 1 }, { background: true }),
      sft_Collection.createIndex({ origin: 1 }, { unique: true, background: true }),
    ]);
...
async function storePosts(postsArr) {
  ...
  try {
    const result = await db.collection("sft-posts").insertMany(posts, { ordered: false });
