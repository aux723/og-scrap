...
async function extractLinkDetails(links, page) {
  ...
  for (let i = 0; i<links.length; i++) {
    try {
      ...
      //post_details[post_details.length] = `Deadline: `+ checkDateValidity.dateformat;

      //let post_d2 = [...post_details];


      /*post_details = post_details.join();
      if (post_details.includes("Fully funded") && post_details.includes("Scholarship name")) {
        post_details = post_details.split(",");
        post_details = post_details.filter(d=>{
          if(!d.includes("Fully funded")) {
            return d;
          }
        })
      } else {
